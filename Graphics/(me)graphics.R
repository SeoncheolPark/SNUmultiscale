...
help(quantile)
precip.testm
#quantile(rain$data[format(index(rain$data),"%m")=="07",2], 0.998, na.rm=T)
########################################
##Monthly prediction lines
########################################
par(mfrow=c(4,3))
par(mar=c(5.1,4.1,4.1,2.1)/5)
TRUEresult <- aggregate(. ~ stations.num + months , data=precip.testm, function(x) quantile(x, prob=0.998,na.rm=T))
#Middle <- read.csv("~/Dropbox/EVA2017/AfterEVA/SNU(m).csv")
Middle <- read.csv("~/Dropbox/EVA2017/AfterEVA/Me/New/middlem(meangrf)500trees1mtry5minnodesize(1019).csv")
Middle2 <- read.csv("~/Dropbox/EVA2017/AfterEVA/Me/New/middlem(mean)500trees4mtry5minnodesize(1019).csv")
colnames(Middle)[1] <- "stations"
Maxval <- max(c(max(Middle[,-1]), max(TRUEresult$prcp)))
#boxplot(as.numeric(rain_training[,5]) ~ reorder(format(date(rain_training),'%m'),rain_training[,5]), outline = FALSE, main="station 5", ) 

plot(TRUEresult$stations.num[TRUEresult$months=="Jan"], TRUEresult$prcp[TRUEresult$months=="Jan"], type='l', xlab="station", ylab="precip", main="JAN", ylim=c(0,Maxval))
lines(sort(Middle$stations),Middle$X1[order(Middle$stations)], col="red")
lines(sort(Middle$stations),Middle2$X1[order(Middle$stations)], col="blue")
points(as.numeric(precip.testm[precip.testm$months=="Jan",]$stations.num), precip.testm[precip.testm$months=="Jan",]$prcp)
legend("topleft", col=c(1,2,4), c("TRUE", "mtry=1", "mtry=4"), lty=c(1,1,1))
plot(TRUEresult$stations.num[TRUEresult$months=="Feb"], TRUEresult$prcp[TRUEresult$months=="Feb"], type='l', xlab="station", ylab="precip", main="FEB", ylim=c(0,Maxval))
lines(sort(Middle$stations),Middle$X2[order(Middle$stations)], col="red")
lines(sort(Middle$stations),Middle2$X2[order(Middle$stations)], col="blue")
points(as.numeric(precip.testm[precip.testm$months=="Feb",]$stations.num), precip.testm[precip.testm$months=="Feb",]$prcp)
plot(TRUEresult$stations.num[TRUEresult$months=="Mar"], TRUEresult$prcp[TRUEresult$months=="Mar"], type='l', xlab="station", ylab="precip", main="MAR", ylim=c(0,Maxval))
lines(sort(Middle$stations),Middle$X3[order(Middle$stations)], col="red")
lines(sort(Middle$stations),Middle2$X3[order(Middle$stations)], col="blue")
points(as.numeric(precip.testm[precip.testm$months=="Mar",]$stations.num), precip.testm[precip.testm$months=="Mar",]$prcp)
plot(TRUEresult$stations.num[TRUEresult$months=="Apr"], TRUEresult$prcp[TRUEresult$months=="Apr"], type='l', xlab="station", ylab="precip", main="APR", ylim=c(0,Maxval))
lines(sort(Middle$stations),Middle$X4[order(Middle$stations)], col="red")
lines(sort(Middle$stations),Middle2$X4[order(Middle$stations)], col="blue")
points(as.numeric(precip.testm[precip.testm$months=="Apr",]$stations.num), precip.testm[precip.testm$months=="Apr",]$prcp)
plot(TRUEresult$stations.num[TRUEresult$months=="May"], TRUEresult$prcp[TRUEresult$months=="May"], type='l', xlab="station", ylab="precip", main="MAY", ylim=c(0,Maxval))
lines(sort(Middle$stations),Middle$X5[order(Middle$stations)], col="red")
lines(sort(Middle$stations),Middle2$X5[order(Middle$stations)], col="blue")
points(as.numeric(precip.testm[precip.testm$months=="May",]$stations.num), precip.testm[precip.testm$months=="May",]$prcp)
plot(TRUEresult$stations.num[TRUEresult$months=="Jun"], TRUEresult$prcp[TRUEresult$months=="Jun"], type='l', xlab="station", ylab="precip", main="JUN", ylim=c(0,Maxval))
lines(sort(Middle$stations),Middle$X6[order(Middle$stations)], col="red")
lines(sort(Middle$stations),Middle2$X6[order(Middle$stations)], col="blue")
points(as.numeric(precip.testm[precip.testm$months=="Jun",]$stations.num), precip.testm[precip.testm$months=="Jun",]$prcp)
plot(TRUEresult$stations.num[TRUEresult$months=="Jul"], TRUEresult$prcp[TRUEresult$months=="Jul"], type='l', xlab="station", ylab="precip", main="JUL", ylim=c(0,Maxval))
lines(sort(Middle$stations),Middle$X7[order(Middle$stations)], col="red")
lines(sort(Middle$stations),Middle2$X7[order(Middle$stations)], col="blue")
points(as.numeric(precip.testm[precip.testm$months=="Jul",]$stations.num), precip.testm[precip.testm$months=="Jul",]$prcp)
plot(TRUEresult$stations.num[TRUEresult$months=="Aug"], TRUEresult$prcp[TRUEresult$months=="Aug"], type='l', xlab="station", ylab="precip", main="AUG", ylim=c(0,Maxval))
lines(sort(Middle$stations),Middle$X8[order(Middle$stations)], col="red")
lines(sort(Middle$stations),Middle2$X8[order(Middle$stations)], col="blue")
points(as.numeric(precip.testm[precip.testm$months=="Aug",]$stations.num), precip.testm[precip.testm$months=="Aug",]$prcp)
plot(TRUEresult$stations.num[TRUEresult$months=="Sep"], TRUEresult$prcp[TRUEresult$months=="Sep"], type='l', xlab="station", ylab="precip", main="SEP", ylim=c(0,Maxval))
lines(sort(Middle$stations),Middle$X9[order(Middle$stations)], col="red")
lines(sort(Middle$stations),Middle2$X9[order(Middle$stations)], col="blue")
points(as.numeric(precip.testm[precip.testm$months=="Sep",]$stations.num), precip.testm[precip.testm$months=="Sep",]$prcp)
plot(TRUEresult$stations.num[TRUEresult$months=="Oct"], TRUEresult$prcp[TRUEresult$months=="Oct"], type='l', xlab="station", ylab="precip", main="OCT", ylim=c(0,Maxval))
lines(sort(Middle$stations),Middle$X10[order(Middle$stations)], col="red")
lines(sort(Middle$stations),Middle2$X10[order(Middle$stations)], col="blue")
points(as.numeric(precip.testm[precip.testm$months=="Oct",]$stations.num), precip.testm[precip.testm$months=="Oct",]$prcp)
plot(TRUEresult$stations.num[TRUEresult$months=="Nov"], TRUEresult$prcp[TRUEresult$months=="Nov"], type='l', xlab="station", ylab="precip", main="NOV", ylim=c(0,Maxval))
lines(sort(Middle$stations),Middle$X11[order(Middle$stations)], col="red")
lines(sort(Middle$stations),Middle2$X11[order(Middle$stations)], col="blue")
points(as.numeric(precip.testm[precip.testm$months=="Nov",]$stations.num), precip.testm[precip.testm$months=="Nov",]$prcp)
plot(TRUEresult$stations.num[TRUEresult$months=="Dec"], TRUEresult$prcp[TRUEresult$months=="Dec"], type='l', xlab="station", ylab="precip", main="DEC", ylim=c(0,Maxval))
lines(sort(Middle$stations),Middle$X12[order(Middle$stations)], col="red")
lines(sort(Middle$stations),Middle2$X12[order(Middle$stations)], col="blue")
points(as.numeric(precip.testm[precip.testm$months=="Dec",]$stations.num), precip.testm[precip.testm$months=="Dec",]$prcp)
...
png("Scatter.png", height=5.4, width=7.8, units = "in", res=100)
##plotting
...
par(mar=c(5.1,2.1,4.1,0.55)/1)
par(mfrow=c(1,1))
boxplot(VIPresulta, xlab="%IncOOBLoss",cex.axis=0.95)
...
prelem <- read.csv("middlem(meangrf)500trees1mtry5minnodesize(efron).csv")

prelem3 <- read.csv("middlem(mean)500trees4mtry5minnodesize(1019).csv")
...
prelem2 <- read.csv("middlem(meannoncircular)500trees1mtry5minnodesize(1025).csv")
...
barplot(t(as.matrix(100*dedata)), main="", ylab="performance(%)", beside=TRUE, col=rainbow(3))
legend("topright", c("CQRF (mtry=1)","LQRF (mtry=1)","CQRF (mtry=4)"), cex=1, fill=rainbow(3))
...
length(stations.test)

#library(ggmap)
#map <- get_map(location = c(mean(range(dfdata[,3])),mean(range(dfdata[,2]))), zoom = 7)
#ggmap(map)

ggplot(data=dfdata, aes(x = stations.long,y = stations.lat)) + geom_bar(aes(plotscore2a[1], plotscore2c[1]), data=dfdata)
...
for(i in 1: nrow(dfdata)){
  if(i==9){
    subplot(barplot(height = as.numeric(as.character(unlist(dfdata[(i+1), 4:6], use.names=F))), axes=F, col=rainbow(3), width=0.05, xlim=c(0,0.4), ylim=c(min(dfdata[,c(4:6)]), -min(dfdata[,c(4:6)])) ), x=dfdata[(i+1),3], y=dfdata[(i+1),2], size=c(0.6,0.6,0.6))
  }else if(i==10){
    subplot(barplot(height = as.numeric(as.character(unlist(dfdata[(i-1), 4:6], use.names=F))), axes=F, col=rainbow(3), width=0.05, xlim=c(0,0.4), ylim=c(min(dfdata[,c(4:6)]), -min(dfdata[,c(4:6)])) ), x=dfdata[(i-1),3], y=dfdata[(i-1),2], size=c(0.6,0.6,0.6))
  }else{
    subplot(barplot(height = as.numeric(as.character(unlist(dfdata[i, 4:6], use.names=F))), axes=F, col=rainbow(3), width=0.05, xlim=c(0,0.4), ylim=c(min(dfdata[,c(4:6)]), -min(dfdata[,c(4:6)])) ), x=dfdata[i,3], y=dfdata[i,2], size=c(0.6,0.6,0.6))
  }
}
par(las=1)
library(scales)
rect(xleft=4.0, xright=4.75, ytop=47.25, ybottom=46.75, border=T)
subplot(barplot(height = 100*as.numeric(as.character(unlist(dfdata[26, 4:6], use.names=F))), axes=T, beside = F, col=rainbow(3), width=0.05, xlim=c(0,0.4), ylim=c(100*min(dfdata[,c(4:6)]), -100*min(dfdata[,c(4:6)])), ylab="%",  cex.axis = 0.75), x=4.674, y=46.992, size=c(0.6,0.6,0.6))

legend("topleft", legend=c("CQRF (mtry=1)","LQRF (mtry=1)","CQRF (mtry=4)"), fill=rainbow(3))
xtab = c(rep(0.05, 4), rep(-0.1,2), rep(0.05,28))
ytab = c(rep(-0.05,3), rep(0.15,1), rep(-0.1,3),rep(0.15,1),rep(-0.05,1), rep(0.15,1),rep(-0.05,24))
...
library(ggplot2)
p <- ggplot(data=dfdata, mapping=aes(x=stations.long, y=stations.lat)) + geom_blank()
p + geom_bar(mapping=aes(x=stations.long[i], y=stations.lat[i]),data=dfdata)
geom_errorbar(data = dfdata[1,c(4:6)], size =3, colour = "brown", alpha = .8, width = 0, aes(x = dfdata$stations.long[1], ymin = dfdata$stations.lat[1]-0.1, ymax = dfdata$stations.lat[1]+0.1))
geom_errorbar(data = cap_bar, size =3, colour = "brown", alpha = .8, width = 0, aes(x = cap_long - 1, ymin = cap_lat, ymax = cap_lat + Y2003 / Y2013 * 3))
