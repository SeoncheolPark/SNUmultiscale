...
##Load Packages
########################################
wants <- c("abind", "animation", "automap", "cluster", "ClusterMax", "fields", "forecast", "geoR", "GeoXp", "ggmap", "ggplot2", "gstat", "lubridate", "mapdata", "maps", "maptools", "nlt" ,"quantregRanger", "rgdal", "rgeos", "reshape", "reshape2", "rrcov", "shapefiles", "sp", "SpatialExtremes", "TSA", "psych", "tseries", "wavethresh", "xts")
...
for(i in 1:N){
  ...
  ##97993개
  set.seed(i)
  training_index <- sort(sample(c(1:nrow(rain_new)), size=nrow(rain_new)*0.8))
  ...
  #rainp <- foreach(j=2:length(date_index), .combine="cbind", .packages=c("quantregForest","reshape2", "lubridate", "gbm") ) %dopar% {
  for(j in 1:1){
    if(j==1){
      ...
      #qrf[[j]] <- quantregForest(x=trainingset[[j]][,-c(1,4,5,6,7)], y=trainingset[[j]][,1], what=c(0.998), ntree=500, nodesize=5, mtry=1)
      qrf[[j]] <- quantregRanger(rain ~ lon + lat + cosmonth + sinmonth, data = trainingset[[j]], params.ranger = list(mtry = 1, importance="permutation", num.trees=500, min.node.size=5))
      ...
      predictQR[[j]]  <- predict(qrf[[j]], predictset[[j]], quantiles=c(0.998))
      ...
      predicttable[[j]] <- acast(predictQR[[j]], stationnum~month, value.var="quantile..0.998")
      sqlosstable[[j]] <- matrix(0, nrow=40, ncol=12); colnames(sqlosstable[[j]])=c(1:12); rownames(sqlosstable[[j]])=c(1:40)
      for(k in 1:nrow(rain_test)){
        sqlosstable[[j]][rain_test$staionnum[k],rain_test$month[k]] <- sqlosstable[[j]][rain_test$staionnum[k],rain_test$month[k]] + sum(qlf(rain_test$rain[k], predicttable[[j]][rain_test$staionnum[k],rain_test$month[k]]), na.rm=T)
      }
      sqlosstable[[j]][sqlosstable[[j]]==0] <- NA
      #list(l=sqlosstable[[j]],p=predicttable[[j]])
      summaryw <- list(l=sqlosstable[[j]],p=predicttable[[j]], q=qrf[[j]])
    }else if(j==2){
      ## circular statistics
      ...
      predicttable[[j]] <- acast(predictQR[[j]], stationnum~month, fun.aggregate=mean)
      sqlosstable[[j]] <- matrix(0, nrow=40, ncol=12); colnames(sqlosstable[[j]])=c(1:12); rownames(sqlosstable[[j]])=c(1:40)
      for(k in 1:nrow(rain_test)){
        sqlosstable[[j]][rain_test$staionnum[k],rain_test$month[k]] <- sqlosstable[[j]][rain_test$staionnum[k],rain_test$month[k]] + sum(qlf(rain_test$rain[k], predicttable[[j]][rain_test$staionnum[k],rain_test$month[k]]), na.rm=T)
      }
  ...
  finaltable[[i]] <- summaryw
  result_combined_small[[i]] <- list(l=finaltable[[i]]$l, r=finaltable[[i]]$p, v=finaltable[[i]]$q$variable.importance, n=finaltable[[i]]$q$num.trees, m=finaltable[[i]]$q$mtry, minnodesize=finaltable[[i]]$q$min.node.size)
  qrf[[j]] <- c()
}
...
saveRDS(result_combined_small, "weightmodel1final100(500trees)mtry1minnodesize5(ranger).RDS")
